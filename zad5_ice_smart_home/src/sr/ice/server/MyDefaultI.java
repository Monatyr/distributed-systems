...
public class MyDefaultI implements Microwave, Fridge {
    ...
    public void setTemp(float newTemp, Current current) throws RangeException { //TODO: setMicrowaveTemp
        if (newTemp > maxTemp || newTemp < minTemp) {
            System.out.println(current.id.category + "/" + current.id.name + " - number out of range in setTemp call: " + newTemp);
            throw (new RangeException(newTemp, minTemp, maxTemp));
        }
        System.out.println(current.id.category + "/" + current.id.name + " - change microwave temperature to: " + newTemp);
    ...
    public float getTemp(Current current) { //TODO: getMicrowaveTemp
        ...
        System.out.println(current.id.category + "/" + current.id.name + " - get microwave temperature: " + microwaveTemps.get(current.id.name));
